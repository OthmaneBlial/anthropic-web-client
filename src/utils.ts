...
export function createMessageElement(
  ...
  messageDiv.className = `p-4 ${
    role === "user" ? "bg-indigo-100" : "bg-white"
  } rounded-lg shadow-md border border-indigo-200`;
  ...
  parts.forEach((part) => {
    if (part.startsWith("```") && part.endsWith("```")) {
      ...
      codeBlockHeader.className =
        "code-block-header flex justify-between items-center bg-gray-200 px-2 py-1 rounded-t";
      ...
      copyButton.className =
        "copy-button bg-indigo-500 text-white px-2 py-1 rounded text-sm";
...
export async function sendMessageToApi(
  apiRequest: AnthropicApiRequest
): Promise<string> {
  try {
    const response = await fetch("https://www.othmaneblial.xyz/anthropic-api", {
      method: "POST",
